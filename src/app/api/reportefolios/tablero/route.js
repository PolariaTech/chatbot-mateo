...
import { consultarVistaFoliosDigitales } from '../../../../lib/reportes-tablero';

const FECHA_ISO = /^\d{4}-\d{2}-\d{2}$/;

function leerEnteros(valor) {
  const lista = Array.isArray(valor) ? valor : valor == null || valor === '' ? [] : [valor];
  return [...new Set(lista.map((v) => Number(v)).filter((n) => Number.isFinite(n)))];
}
...
export async function POST(request) {
  ...
  const filtrosEq = {
    estado_contable: leerEnteros(body.estado_contable),
    cancelado: leerEnteros(body.cancelado),
    uuid_estado: leerEnteros(body.uuid_estado),
  };

  try {
    const { rows, total } = await consultarVistaFoliosDigitales({
      schema,
      fechaInicio,
      fechaFin,
      limite,
